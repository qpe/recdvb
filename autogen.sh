...
[ -f configure.ac ] || {
  echo "autogen.sh: run this command only at the top of a recdvb source tree."
  exit 1
}
...
(autoconf --version) < /dev/null > /dev/null 2>&1 || {
  echo
  echo "You must have autoconf installed to compile recdvb."
...
(automake --version) < /dev/null > /dev/null 2>&1 || {
  echo
  echo "You must have automake installed to compile recdvb."
...
echo "Generating configure script and Makefiles for recdvb."
